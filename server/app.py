...
def bakery_by_id(id):
    ...
    if request.method == 'GET':
        bakery_serialized = bakery.to_dict(rules=('-baked_goods',))
...
def baked_goods():
    ...
    if request.method == 'GET':
        ...
        new_bakedgood = BakedGood(
            ...
            price = request.form.get('comment'),
        ...
        response_dict = new_bakedgood.to_dict()
...
def baked_goods_delete(id):
    ...
    if baked_good == None:
        error = {"message": "BakedGood id not available"}
        response = make_response(jsonify(error), 400)

        return response
    else:
        if request.method == 'GET':
            response_dict = baked_good.to_dict()
            response = make_response(response_dict, 200)
            ...
            message = {'message': 'Object deleted successfully'}
